...
#ifdef CONFIG_RECV_MGMT
typedef void (* mwifi_monitor_cb_t)(uint8_t *data, int len);

typedef struct {
    bool mgmt_recv_enable;
    mwifi_monitor_cb_t cb;
} wifi_recv_mgmt_t;

int wifi_start_recv_mgmt(mwifi_monitor_cb_t cb, int ch);
int wifi_stop_recv_mgmt();
int process_mgmt_frame(uint8_t * data, uint16_t len,uint32_t recvec1b);
#endif
...
sys_err_t wifi_clean_sta_nv_info(void);
